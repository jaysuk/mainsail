...
/** Replaces the Vue 2 `SettingsGeneralDatabase` class component. */
export function useSettingsGeneralDatabase() {
    const { t } = useI18n()
    ...
    const sortNamespaces = (a: { value: string; label: string }, b: { value: string; label: string }) => {
    ...
    const loadBackupableNamespaces = async () => {
        let backupableNamespaces: { value: string; label: string }[] = []

        const urlRequestDbList = socketStore.getUrl + '/server/database/list'
        ...
        if (availableNamespaces.includes('mainsail')) {
            const urlRequestMainsailNamespace = socketStore.getUrl + '/server/database/item?namespace=mainsail'
        ...
        if (availableNamespaces.includes('maintenance')) {
            backupableNamespaces.push({ value: 'maintenance', label: t('Settings.GeneralTab.DbMaintenance') })
        }

        if (availableNamespaces.includes('timelapse')) {
            backupableNamespaces.push({ value: 'timelapse', label: t('Settings.GeneralTab.DbTimelapseSettings') })
        }

        if (availableNamespaces.includes('webcams')) {
            backupableNamespaces.push({ value: 'webcams', label: t('Settings.WebcamsTab.Webcams') })
        }

        return backupableNamespaces
    }

    return { availableKeys, loadBackupableNamespaces, sortNamespaces }
}
