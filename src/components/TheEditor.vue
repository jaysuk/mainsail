<template>
    <div>
        <v-dialog
            ...
            <panel card-class="editor-dialog" :icon="isWriteable ? mdiFileDocumentEditOutline : mdiFileDocumentOutline" :title="title">
                <template #buttons>
                    ...
                    <v-btn v-if="restartServiceName === 'klipper'" variant="text" tile :href="klipperConfigReference" target="_blank" class="d-none d-md-flex">
                    ...
                    <v-btn v-if="restartServiceNameExists" color="primary" variant="text" tile class="d-none d-sm-flex" @click="save(restartServiceName)">
                ...
                <v-card-text class="pa-0 d-flex">
                    ...
                    <div v-if="existsFileStructure && fileStructureSidebar" class="d-none d-md-flex structure-sidebar">
                        <v-treeview v-model:activated="structureActive" v-model:opened="structureOpen" activatable density="compact" item-title="name" item-value="line" :items="configFileStructure" class="w-100" @update:activated="activeChanges">
                            <template #title="{ item }">
                                <div class="cursor-pointer _structure-sidebar-item" :class="item.type == 'item' ? 'ͼp' : 'ͼt'" @click="activeChangesItemClick">
                                    {{ item.name }}
                                </div>
                            </template>
                            <template v-if="restartServiceName === 'klipper'" #append="{ item }">
                                <v-btn v-if="item.type == 'section'" icon size="small" variant="plain" color="grey-darken-2" :href="klipperConfigReference + '#' + item.name.split(' ')[0]" target="_blank">
        ...
        <v-snackbar v-model="loaderBool" :timeout="-1" location="bottom right">
            ...
            <template v-if="loaderProgress.total > 0">
                ...
                <br />
                <v-progress-linear class="mt-2" :model-value="(100 * loaderProgress.loaded) / loaderProgress.total" />
            </template>
            <template v-else>
                <v-progress-linear class="mt-2" indeterminate />
            </template>
            <template #actions>
                <v-btn color="red" variant="text" style="min-width: auto" tile @click="cancelDownload">
                    <v-icon>{{ mdiClose }}</v-icon>
                </v-btn>
            </template>
        </v-snackbar>
        <v-dialog v-model="dialogConfirmChange" persistent :width="600">
            <panel card-class="editor-confirm-change-dialog" :icon="mdiHelpCircle" :title="t('Editor.UnsavedChanges')" :margin-bottom="false">
                <template #buttons>
                ...
                <v-card-actions>
                    ...
                    <v-btn variant="text" color="primary" @click="save(null)">
                        {{ t('Editor.SaveClose') }}
                    </v-btn>
...
import { mdiClose, mdiCloseThick, mdiContentSave, mdiFileDocumentOutline, mdiFileDocumentEditOutline, mdiHelp, mdiHelpCircle, mdiRestart, mdiUsb, mdiFormatListCheckbox } from '@mdi/js'
import DevicesDialog from '@/components/dialogs/DevicesDialog.vue'
import type { ConfigFileSection } from '@/store/files/types'
...
const { printer_state, klipperConfigReference } = useBase()
...
let structureActiveChangedBySidebar = false

const editor = ref<InstanceType<typeof Codemirror> | null>(null)

const changed = computed(() => editorStore.changed ?? false)
const changedOutput = computed(() => (changed.value ? '*' : ''))

const show = computed(() => editorStore.bool ?? false)

const filepath = computed(() => editorStore.filepath ?? '')
const filename = computed(() => editorStore.filename ?? '')
...
const permissions = computed(() => editorStore.permissions ?? 'r')
const isWriteable = computed(() => permissions.value.includes('w'))

const sourcecode = computed({
    get: () => editorStore.sourcecode ?? '',
    set: (newVal) => editorStore.updateSourcecode(newVal),
})

const loaderBool = computed(() => editorStore.loaderBool ?? false)
const loaderProgress = computed(() => editorStore.loaderProgress ?? {})
...
const availableServices = computed(() => serverStore.system_info?.available_services ?? [])
...
const restartServiceName = computed(() => {
    if (availableServices.value.includes(filenameWithoutExtension.value) && fileExtension.value === 'conf') return filenameWithoutExtension.value
...
const confirmUnsavedChanges = computed(() => guiStore.editor.confirmUnsavedChanges ?? false)
const escToClose = computed(() => guiStore.editor.escToClose ?? false)

const title = computed(() => {
    const titleValue = filepath.value ? `${filepath.value}/${filename.value}` : filename.value

    if (!isWriteable.value) return `${titleValue} (${t('Editor.FileReadOnly')})`

    return `${titleValue} ${changedOutput.value}`
})

const fileStructureSidebar = computed({
    get: () => guiStore.editor.fileStructureSidebar,
    set: (newVal) => guiStore.saveSetting({ name: 'editor.fileStructureSidebar', value: newVal }),
})
...
function promptUnsavedChanges() {
    if (!changed.value || !isWriteable.value) editorStore.close()
    else dialogConfirmChange.value = true
}

function save(restartServiceNameArg: string | null = null) {
    dialogConfirmChange.value = false

    editorStore.saveFile({
        content: sourcecode.value,
        restartServiceName: restartServiceNameArg,
    })
}

// Relies on event bubbling to flip the flag before treeview active change is handled
function activeChangesItemClick() {
    structureActiveChangedBySidebar = true
}

function activeChanges(activeItems: number[]) {
    if (!structureActiveChangedBySidebar) {
        return
    }
