<template>
    <e-chart ref="historyFilamentUsage" :option="chartOptions" :autoresize="true" :init-options="{ renderer: 'svg' }" style="height: 175px; width: 100%"></e-chart>
</template>
...
const { fgColorLow, fgColorMid } = useMainsailTheme()
const { allJobs, selectedJobs } = useHistory()

const historyFilamentUsage = ref<EChartRef | undefined>()
let observer: IntersectionObserver | null = null

const chart = computed(() => historyFilamentUsage.value?.chart ?? null)

function formatTooltip(params: TopLevelFormatterParams): string {
    const entry = Array.isArray(params) ? params[0] : params
    if (!Array.isArray(entry?.data) || typeof entry.data[0] !== 'number' || typeof entry.data[1] !== 'number') return ''
...
const filamentUsageArray = computed<[number, number][]>(() => {
    ...
    let jobsFiltered = [...allJobs.value.filter((job: ServerHistoryStateJob) => new Date(job.start_time * 1000) >= startDate && job.filament_used > 0)]
    if (selectedJobs.value.length)
        jobsFiltered = [...selectedJobs.value.filter((job: ServerHistoryStateJob) => new Date(job.start_time * 1000) >= startDate && job.filament_used > 0)]
...
onMounted(() => {
    ...
    if (historyFilamentUsage.value?.$el) observer.observe(historyFilamentUsage.value.$el)
})
...
watch(filamentUsageArray, (newVal) => {
