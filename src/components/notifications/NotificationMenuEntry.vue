        ...
        <v-row align="start" class="flex-nowrap">
            <v-col class="grow pb-2">
                <div class="notification-menu-entry__headline mb-1 text-subtitle-1">
                    <a v-if="'url' in entry" :class="`text-decoration-none text-${alertColor} `" :href="entry.url" target="_blank">
                ...
                <p class="notification-menu-entry__description text-body-2 mb-0 text-disabled font-weight-light" v-html="formatedText" />
                <v-btn v-if="entryType === 'maintenance'" variant="outlined" size="small" :color="alertColor" class="mt-3 mb-0 w-100" @click="showMaintenanceDetails = true">
                    {{ t('App.Notifications.ShowDetails') }}
                </v-btn>
            </v-col>
            <v-col v-if="entry.priority !== 'critical'" class="shrink pl-0 pb-1 pt-1 pr-2 d-flex flex-column align-self-stretch justify-space-between">
                <v-btn v-if="entryType !== 'maintenance'" icon variant="plain" :color="alertColor" class="mb-2" @click="xButtonAction">
                    <v-icon>{{ mdiClose }}</v-icon>
                </v-btn>
                <v-spacer />
                <v-btn icon variant="plain" retain-focus-on-click :color="alertColor" @click="expand = !expand">
...
const formatedText = computed(() =>
    props.entry.description.replace(
        ...
        '<a href="$1" target="_blank" class="text-' + alertColor.value + '">$1</a>'
    )
)
...
const maintenanceEntry = computed(() => {
    ...
    return guiMaintenanceStore.getEntries.find((entry) => entry.id === id) ?? null
})

const reminderTimes = computed<ReminderOption[]>(() => {
...
function close() {
    guiNotificationsStore.close({ id: props.entry.id })
}

function dismiss(type: 'time' | 'reboot', time: number | null) {
    guiNotificationsStore.dismiss({ id: props.entry.id, type, time })
}
