<template>
    <panel v-if="is_active" :title="t('Heightmap.CurrentMesh.Headline')" card-class="heightmap-current-mesh-panel" :icon="mdiInformation" :collapsible="true" class="mt-0">
        <v-card-text class="py-3 px-0">
            <v-row class="px-3">
                <v-col>{{ t('Heightmap.CurrentMesh.Name') }}</v-col>
                <v-col class="text-right">
                    <span v-if="!name.startsWith('adaptive-')" class="currentMeshName cursor-pointer font-weight-bold" @click="showRename = true">
                        <v-icon start size="small" color="primary">{{ mdiPencil }}</v-icon>
                        {{ name }}
                    </span>
            ...
            <v-row v-if="index_max > -1" class="px-3">
                <v-col> {{ t('Heightmap.CurrentMesh.Max') }} [{{ position_max_x.toFixed(1) }}, {{ position_max_y.toFixed(1) }}] </v-col>
                <v-col class="text-right">{{ max.toFixed(3) }} mm</v-col>
            </v-row>
            <v-divider class="my-3" />
            <v-row class="px-3">
                <v-col> {{ t('Heightmap.CurrentMesh.Min') }} [{{ position_min_x.toFixed(1) }}, {{ position_min_y.toFixed(1) }}] </v-col>
...
const { t } = useI18n()
const { bed_mesh, mesh_min, mesh_max, min, max, variance, is_active, name, points } = useBedmesh()
