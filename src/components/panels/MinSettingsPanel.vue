<template>
    <panel v-if="klipperState === 'ready' && existsPrinterConfig && missingConfigs.length" :icon="mdiAlertCircle" :title="t('Panels.MinSettingsPanel.MissingConfiguration')" :collapsible="true" card-class="min-settings-panel" toolbar-color="orange-darken-2">
        <v-card-text>
            <v-row>
                <v-col>
                    <ul class="">
...
const existsPrinterConfig = computed(() => printerStore.existPrinterConfig ?? false)

const missingConfigs = computed(() => printerStore.checkNecessaryConfig ?? [])

const mainsailCfgExists = computed(() => filesStore.checkConfigFile('mainsail.cfg') ?? false)
</script>
