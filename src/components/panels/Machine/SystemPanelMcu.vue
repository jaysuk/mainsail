        ...
        <v-row class="py-0 pr-4">
            <v-col class="pl-6">
                ...
                <div class="text-body-2">
                    ...
                    <div>
                        <span class="text-no-wrap"> {{ t('Machine.SystemPanel.Values.Load', { load: mcu.load }) }}, </span>
                        <span class="text-no-wrap"> {{ t('Machine.SystemPanel.Values.Awake', { awake: mcu.awake }) }}, </span>
                        <span v-if="mcu.freq !== null" class="text-no-wrap"> {{ t('Machine.SystemPanel.Values.Frequency', { frequency: mcu.freqFormat }) }}, </span>
                                ...
                                <v-tooltip location="top">
                                    <template #activator="{ props: activatorProps }">
                                        <span v-bind="activatorProps">
                                            {{ t('Machine.SystemPanel.Values.Temp', { temp: mcu.tempSensor.temperature }) }}
                                        </span>
                                    </template>
                                    <span>
                                        {{ t('Machine.SystemPanel.Values.TempMax', { temp: mcu.tempSensor.measured_max_temp }) }}
                                        <br />
                                        {{ t('Machine.SystemPanel.Values.TempMin', { temp: mcu.tempSensor.measured_min_temp }) }}
                                    </span>
                                </v-tooltip>
                            </template>
            ...
            <v-col class="px-2 col-auto d-flex justify-center align-center">
                <v-progress-circular :rotate="-90" :size="55" :width="7" :model-value="mcu.loadPercent" :color="mcu.loadProgressColor">
                    {{ mcu.loadPercent }}
                </v-progress-circular>
            </v-col>
        </v-row>
        <v-dialog v-model="mcuDetailsDialog" :max-width="400" :max-height="500" scrollable>
            <panel :title="mcu.name" icon="mdi-text-box-search-outline" card-class="machine-systemload-mcu-details-dialog" :margin-bottom="false">
                <template #buttons>
...
defineProps<{ mcu: PrinterStateMcu }>()
