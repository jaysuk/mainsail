    ...
    <li class="commit px-3 py-2">
        <v-row class="flex-column flex-sm-row">
            <v-col>
                ...
                <p v-if="showDetails" class="caption text-medium-emphasis mb-2" style="white-space: pre-line" v-html="message"></p>
...
import type { ServerUpdateManagerStateGitRepo, ServerUpdateManagerStateGitRepoCommit } from '@/store/server/updateManager/types'
...
const commitHref = computed(() => `https://github.com/${props.repo.owner}/${repo_name.value}/commit/${props.commit.sha}`)
