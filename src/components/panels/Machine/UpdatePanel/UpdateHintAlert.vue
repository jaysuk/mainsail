<template>
    <v-alert variant="text" density="compact" border="start" color="orange" :icon="mdiAlertCircle">
        ...
        <div class="text-center">
            <v-btn v-if="configured_type === 'git_repo' && repo.commits_behind?.length" class="mx-2 mt-3 mt-sm-0" @click="openCommitHistory">
...
const emit = defineEmits<{ 'open-commit-history': [] }>()
...
const externalLink = computed(() => {
    ...
    if (props.repo?.configured_type === 'web') return `//github.com/${props.repo.owner}/${repo_name.value}/releases/tag/${props.repo.remote_version}`
