    ...
    <div>
        <v-row class="py-2">
            ...
            <v-col class="col-auto pr-6 text-right" align-self="center">
                <v-chip v-if="anomalies.length > 0" size="small" label :variant="!toggleAnomalies ? 'outlined' : undefined" color="grey" class="minwidth-0 px-1 mr-2" @click="toggleAnomalies = !toggleAnomalies">
                    <v-icon size="small">{{ toggleAnomalies ? mdiInformationOutline : mdiInformation }}</v-icon>
                </v-chip>
                <template v-if="!isValid">
                    <v-menu location="bottom end">
                        <template #activator="{ props: activatorProps }">
                            <v-chip size="small" label variant="outlined" :color="btnColor" :disabled="btnDisabled" class="minwidth-0 px-2 text-uppercase" v-bind="activatorProps">
                        ...
                        <v-list density="compact" class="py-0">
                            <v-list-item v-if="!isCorrupt" :prepend-icon="mdiReload" :title="t('Machine.UpdatePanel.SoftRecovery')" @click="doRecovery(false)" />
                            <v-list-item :disabled="!existsRecoveryUrl" :prepend-icon="mdiReload" :title="t('Machine.UpdatePanel.HardRecovery')" @click="doRecovery(true)" />
                        </v-list>
                    </v-menu>
                </template>
                <v-chip v-else size="small" label variant="outlined" :color="btnColor" :disabled="btnDisabled" class="minwidth-0 px-2 text-uppercase" @click="clickUpdate">
                    <v-icon size="small" class="mr-1">{{ btnIcon }}</v-icon>
                    {{ btnText }}
                </v-chip>
            </v-col>
        ...
        <v-row v-if="warnings.length" class="mt-0">
            <v-col class="px-6 pt-0">
                <v-alert v-for="(message, index) in warnings" :key="'warnings_' + index" density="compact" variant="text" color="orange" border="start" :icon="mdiCloseCircle">
        ...
        <v-row v-show="toggleAnomalies" class="mt-0">
            <v-col class="px-6 pt-0">
                <v-alert v-for="(message, index) in anomalies" :key="'anomalies_' + index" density="compact" variant="text" color="grey" border="start" :icon="mdiInformation">
                    {{ message }}
                </v-alert>
            </v-col>
        </v-row>
...
import { mdiCloseCircle, mdiCheck, mdiHelpCircleOutline, mdiInformation, mdiInformationOutline, mdiMenuDown, mdiProgressUpload, mdiReload, mdiUpdate } from '@mdi/js'
...
const props = defineProps<{ repo: ServerUpdateManagerStateGitRepo }>()
...
const isSemverType = computed(() => ['executable', 'python', 'web', 'zip'].includes(type.value))
...
const versionOutput = computed(() => {
    ...
    if (commitsBehind.value.length) {
        const tmp = t('Machine.UpdatePanel.CommitsAvailable', { count: commitsBehind.value.length }, commitsBehind.value.length)
...
const pythonChangelog = computed(() => {
    if (props.repo.channel === 'dev') return `${githubRepoUrl.value}/compare/${props.repo.current_hash}..${props.repo.remote_hash}`
...
function doUpdate() {
    if (['klipper', 'moonraker'].includes(props.repo.name)) {
        webSocketClient.emit('machine.update.' + props.repo.name, {})
        return
    }

    webSocketClient.emit('machine.update.client', { name: props.repo.name })
}

function doRecovery(hard: boolean) {
    webSocketClient.emit('machine.update.recover', { name: props.repo.name, hard: hard })
}
</script>
