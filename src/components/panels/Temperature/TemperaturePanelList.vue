    ...
    <responsive
        ...
        <template #default="{ el }">
            <v-table class="temperature-panel-table">
                ...
                <tbody>
                    <temperature-panel-list-item v-for="objectName in heaterObjects" :key="objectName" :object-name="objectName" :input-digits="inputFieldDigits" :is-responsive-mobile="el.is.mobile ?? false" />
                    <temperature-panel-list-item-nevermore v-for="objectName in nevermoreObjects" :key="objectName" :object-name="objectName" :is-responsive-mobile="el.is.mobile ?? false" />
                    <temperature-panel-list-item v-for="objectName in temperature_sensors" :key="objectName" :object-name="objectName" :is-responsive-mobile="el.is.mobile ?? false" />
                    <template v-if="!hideMonitors">
                        <temperature-panel-list-item v-for="objectName in monitors" :key="objectName" :object-name="objectName" :is-responsive-mobile="el.is.mobile ?? false" />
                    </template>
                </tbody>
            </v-table>
        </template>
...
const available_heaters = computed<string[]>(() => printerStore.heaters?.available_heaters ?? [])

const filteredHeaters = computed(() => filterNamesAndSort(available_heaters.value))

const available_sensors = computed<string[]>(() => printerStore.heaters?.available_sensors ?? [])

const available_monitors = computed<string[]>(() => printerStore.heaters?.available_monitors ?? [])

const available_nevermores = computed(() => Object.keys(printerStore).filter((name) => name.startsWith('nevermore')))

const monitors = computed(() => [...available_monitors.value].sort(sortObjectName))

const temperature_fans = computed(() => available_sensors.value.filter((name: string) => name.startsWith('temperature_fan') && !name.startsWith('temperature_fan _')).sort(sortObjectName))

const hideMcuHostSensors = computed<boolean>(() => guiStore.view.tempchart.hideMcuHostSensors ?? false)

const hideMonitors = computed<boolean>(() => guiStore.view.tempchart.hideMonitors ?? false)
