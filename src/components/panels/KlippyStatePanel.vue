        ...
        <v-alert v-if="klippyIsConnected" :color="messageType.color" density="compact" variant="text" border="start" class="mb-0 mb-6">
            ...
            <div v-if="klippy_message !== null">
                ...
                <v-row>
                    ...
                    <v-col>
                        <v-btn :href="apiUrl + '/server/files/klippy.log'" size="small" variant="outlined" :class="buttonClasses" @click="downloadLog">
                            <v-icon class="mr-2">{{ mdiDownload }}</v-icon>
                            {{ t('Panels.KlippyStatePanel.KlipperLog') }}
                        </v-btn>
                        <v-btn :href="apiUrl + '/server/files/moonraker.log'" size="small" variant="outlined" :class="buttonClasses" @click="downloadLog">
                            <v-icon class="mr-2">{{ mdiDownload }}</v-icon>
...
import { mdiRestart, mdiDownload, mdiMessageOutline, mdiAlertOutline, mdiRocketLaunch, mdiConnection, mdiPower } from '@mdi/js'
import { useBase } from '@/composables/useBase'
import { useServerStore } from '@/store/server'
import { webSocketClient } from '@/plugins/webSocketClient'

const { t } = useI18n()
const { apiUrl, socketIsConnected, klippyIsConnected, klipperState, printerPowerDevice, isPrinterPowerOff } = useBase()
const serverStore = useServerStore()

const klippy_message = computed<string | null>(() => serverStore.klippy_message ?? null)
...
const buttonClasses = computed(() => [`text-${messageType.value.color}`, 'my-1', 'w-100'])

const serviceReportsKlipper = computed(() => `${t('Panels.KlippyStatePanel.ServiceReports', { service: 'Klipper' })}: ${klipperState.value.toUpperCase()}`)

const serviceReportsMoonraker = computed(() => `${t('Panels.KlippyStatePanel.ServiceReports', { service: 'Moonraker' })}: ${klipperState.value.toUpperCase()}`)

function restart() {
    webSocketClient.emit('printer.restart', {}, { loading: 'restart' })
}

function firmwareRestart() {
    webSocketClient.emit('printer.firmware_restart', {}, { loading: 'firmwareRestart' })
}
