<template>
    <v-dialog v-model="showDialog" :max-width="600" persistent @keydown.esc="closeDialog">
        <panel :title="t('History.EditMaintenance')" :icon="mdiNotebook" card-class="history-edit-maintenance-dialog" :margin-bottom="false">
            <template #buttons>
            ...
            <v-card-text class="pb-0">
                <v-row>
                    <v-col>
                        <v-text-field v-model="name" :rules="nameInputRules" :label="t('History.Name')" hide-details="auto" variant="outlined" density="compact" />
                    </v-col>
                </v-row>
                <v-row>
                    <v-col>
                ...
                <v-row>
                    <v-col>
                        <settings-row :title="t('History.Reminder')">
                            <v-select v-model="reminder" :items="reminderItems" :disabled="item.end_time !== null" variant="outlined" density="compact" hide-details class="mt-0" />
                        </settings-row>
                    </v-col>
                </v-row>
                <template v-if="reminder">
                    <v-row>
                        <v-col>
                            <settings-row :icon="mdiAdjust" :title="t('History.FilamentBasedReminder')" :sub-title="t('History.FilamentBasedReminderDescription')">
                                <v-checkbox v-model="reminderFilament" :disabled="item.end_time !== null" hide-details class="mt-0" />
                                <v-text-field v-model.number="reminderFilamentValue" :disabled="item.end_time !== null" hide-details="auto" type="number" class="mt-0" variant="outlined" density="compact" :suffix="t('History.Meter')" />
                            </settings-row>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col>
                            <settings-row :icon="mdiAlarm" :title="t('History.PrinttimeBasedReminder')" :sub-title="t('History.PrinttimeBasedReminderDescription')">
                                <v-checkbox v-model="reminderPrinttime" :disabled="item.end_time !== null" hide-details class="mt-0" />
                                <v-text-field v-model.number="reminderPrinttimeValue" :disabled="item.end_time !== null" hide-details="auto" type="number" class="mt-0" variant="outlined" density="compact" :suffix="t('History.Hours')" />
                            </settings-row>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col>
                            <settings-row :icon="mdiCalendar" :title="t('History.DateBasedReminder')" :sub-title="t('History.DateBasedReminderDescription')">
                                <v-checkbox v-model="reminderDate" :disabled="item.end_time !== null" hide-details class="mt-0" />
                                <v-text-field v-model.number="reminderDateValue" :disabled="item.end_time !== null" hide-details="auto" type="number" class="mt-0" variant="outlined" density="compact" :suffix="t('History.Days')" />
                            </settings-row>
                        </v-col>
                    </v-row>
                </template>
...
function save() {
    ...
    guiMaintenanceStore.update(item as GuiMaintenanceStateEntry & { id: string })
