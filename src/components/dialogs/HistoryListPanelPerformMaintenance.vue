<template>
    <v-dialog v-model="showDialog" :max-width="400" persistent @keydown.esc="closeDialog">
        <panel :title="t('History.PerformMaintenance')" :icon="mdiNotebook" card-class="history-perform-maintenance-dialog" :margin-bottom="false">
            <template #buttons>
...
function perform() {
    guiMaintenanceStore.perform({ id: props.item.id as string, note: note.value })
