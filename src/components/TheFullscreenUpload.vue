...
const currentPathGcodes = computed(() => guiStore.view.gcodefiles.currentPath ?? '')
const currentPathConfig = computed(() => guiStore.view.configfiles.currentPath ?? '')
...
async function onDrop(e: DragEvent) {
    ...
    if (e.dataTransfer?.files?.length) {
        ...
        for (const file of files) {
            ...
            const result = await filesStore.uploadFile({ file, path, root })
